...
import { CreatePurchaseOrderDetailRequest, PurchaseOrderDetailResponse } from '@/@core/purchase-data-types'
import { ProductResponse } from '@/@core/master-data-types'
import FormaterHelper from '@/@core/utils/formatHelper'
import FormProductModal from '@/components/modals/form-product'
...
interface PropsViewAddDetailPurchaseOrder {
...
const ViewAddDetailPurchaseOrder: React.FC<PropsViewAddDetailPurchaseOrder> = ({
  detailTransaction,
  visibleAddItem,
  successUpdate,
  number,
  ...
  const [isSuccessUpdateDataProduct, setIsSuccessUpdateDataProduct] = useState<ResponseStatus | null>(null)
  ...
  async function fetchDataForItems() {
    if (visibleAddItem) {
      setShowRowAdd(true)
      setAction('add')
    }
  ...
  const handleInputChangeProduct = (event: SelectChangeEvent<number>) => {
    ...
    if (value === 'add-product') {
      setOpenModalProduct(true) // Open modal when 'Tambah Produk' is selected
  ...
  const successUpdateDataProduct = (response: ResponseStatus) => {
    setIsSuccessUpdateDataProduct(response)
  }

  useEffect(() => {
    if (isSuccessUpdateDataProduct) {
      setSnackBarOpen(true) // Show Snackbar when isSuccessUpdateDataProduct changes
      fetchDataForItems()
    }
  }, [isSuccessUpdateDataProduct])

  const responseAddProduct = (response: ProductResponse) => {
    console.log('response add product : ')
    console.log(response)
    setProductSelected(Number(response.id))
  }
  ...
  return (
    <>
      <CustomSnackBarNotification
        open={snackBarOpen}
        response={isSuccessUpdateDataProduct}
        onClose={handleCloseSnack}
        onResponse={setIsSuccessUpdateDataProduct}
      />
              ...
              <TableRow>
                ...
                <TableCell>
                  <TextField
                    name='total'
                    type='number'
                    value={FormaterHelper.formatRupiah(String(totalPrice))}
                    size='small'
                    aria-readonly
                  />
                </TableCell>
      ...
      {openModalProduct && (
        <FormProductModal
          ...
          successUpdateDataProduct={successUpdateDataProduct}
          responseAddProduct={responseAddProduct}
        />
      )}
    </>
  )
...
export default ViewAddDetailPurchaseOrder
