...
import { Skeleton, TextField, Typography, useTheme, Box, Button } from '@mui/material'
...
import { AcceptedOrderResponse } from '@/@core/accepted-data-types'
...
const columns: GridColDef[] = [
  {
    ...
    width: 150,
    renderCell: params => (
      <Link href={`/purchases/accepted-orders/detail/${params.value}`} passHref>
  ...
  { field: 'description', headerName: 'Deskripsi', width: 160 },
  { field: 'total_qty_received', headerName: 'Total Qty', width: 160, type: 'number' },
...
interface ViewAcceptedOrderProps {
  titlePage: string
}

const ViewAcceptedOrder = ({ titlePage }: ViewAcceptedOrderProps) => {
  ...
  const [acceptedOrderData, setAcceptedOrderData] = useState<AcceptedOrderResponse[]>([])
  ...
  const fetchItems = async () => {
    ...
    try {
      const result = await AppService.serviceGet('api/all-accepted-order', params)
      setAcceptedOrderData(result.data)
    } catch (error) {
  ...
  const filteredData = Array.isArray(acceptedOrderData)
    ? acceptedOrderData.filter(product => product.number.includes(searchQuery.toLowerCase()))
    : [] // Add fallback to an empty array if acceptedOrderData is not an array
