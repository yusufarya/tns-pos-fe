...
interface ModalDeleteItemAOProps {
  open: boolean
  idItemAO: number | undefined
  rowData: any
  handleClose: () => void
  successUpdate: (response: ResponseStatus) => void // Updated to callback
}

const ModalDeleteItemAO: React.FC<ModalDeleteItemAOProps> = ({
  open,
  idItemAO,
  rowData,
  handleClose,
  ...
  const handleDeleteItem = async () => {
    try {
      const response = await AppService.serviceDelete('api/accepted-order-detail/delete', {
        id: idItemAO,
        type: 'single'
      })
...
export default ModalDeleteItemAO
